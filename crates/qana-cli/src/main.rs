...
const USAGE: &str = "\
qana — envelope-certified incremental parsing

USAGE
    qana <command> [options]
...
fn main() {
    ...
    match cmd {
        ...
        other => {
            eprintln!("qana: unknown command `{other}`\n");
            eprint!("{USAGE}");
            std::process::exit(2);
        }
    }
...
fn read_file(path: &str) -> String {
    std::fs::read_to_string(path).unwrap_or_else(|e| {
        eprintln!("qana: cannot read {path}: {e}");
        std::process::exit(2);
    })
}
...
/// gate `qana check` reports on — the other commands just need it to
...
fn load(path: &str) -> Lang {
    ...
    let syntax = qana_services::diagnostics(&tc.lexer, &session.buf, &tc.sg, &session.last_repairs);
    for d in &syntax {
...
fn cmd_new(args: &Args) {
    ...
    if root.exists() && root.read_dir().map(|mut d| d.next().is_some()).unwrap_or(false) {
        eprintln!("qana: {dir} already exists and is not empty");
        std::process::exit(2);
    }
    std::fs::create_dir_all(root.join(".vscode")).unwrap_or_else(|e| {
        eprintln!("qana: cannot create {dir}: {e}");
        std::process::exit(2);
    });
    ...
    println!();
    println!("Next:");
    println!("  qana check {}", g_path.display());
    println!("  qana parse {} {}", g_path.display(), d_path.display());
    println!("  qana defs  {} {}", g_path.display(), d_path.display());
}

fn write_file(path: &std::path::Path, body: &str) {
    std::fs::write(path, body).unwrap_or_else(|e| {
        eprintln!("qana: cannot write {}: {e}", path.display());
        std::process::exit(2);
    });
}
...
fn cmd_check(args: &Args) {
    ...
    row(
        ...
        &{
            let imports =
                def.binding.refs.iter().filter(|r| r.3 == qana_sem::RefKind::Import).count();
...
fn cmd_parse(args: &Args) {
    ...
    let diags = qana_services::diagnostics(&lang.lexer, &session.buf, &lang.def.sg, &session.last_repairs);
    if diags.is_empty() {
...
fn cmd_outline(args: &Args) {
    ...
    let session = session_for(&lang, &text);
    let s = Src::new(doc_path, &text);
    let syms = qana_services::outline(session.tree().expect("total"), &lang.def.outline);
...
fn cmd_expand(args: &Args) {
    ...
    if args.has("check") {
        ...
        if disk_exp == out.text && disk_prov == prov {
            ...
            println!(
                "{} {} drifted from its source — regenerate with `qana expand`",
                red("✗"),
                exp_path.display()
            );
            std::process::exit(1);
...
fn cmd_edit(args: &Args) {
    ...
    let outcome = match session.edit(&lang.def.sg, &lang.tables, &edits) {
        Ok(o) => o,
        Err(e) => {
            eprintln!("qana: incremental parse failed: {e:?}");
            std::process::exit(1);
        }
    };
...
fn cmd_ts(args: &Args) {
    ...
    std::fs::create_dir_all(root.join("queries")).unwrap_or_else(|e| {
        eprintln!("qana: cannot create {outdir}: {e}");
        std::process::exit(2);
    });

...
fn assert_gate(session: &IncSession<'_>, sg: &SynGrammar, tables: &LrTables) {
    use qana_grammar::green::{check_balance, semantic_eq};
...
fn first_diff(
    a: &qana_grammar::GreenNode,
    b: &qana_grammar::GreenNode,
    path: String,
) -> String {
    use qana_grammar::green::flat_children;
    ...
    for (i, (x, y)) in fa.iter().zip(&fb).enumerate() {
        match (x, y) {
            ...
            (GreenChild::Node(m), GreenChild::Node(n)) => {
                if !qana_grammar::green::semantic_eq(m, n) {
...
fn kind_of(c: &qana_grammar::GreenChild) -> String {
    match c {
        qana_grammar::GreenChild::Token(t) => format!("T{}:{:?}", t.id, t.text),
        qana_grammar::GreenChild::Node(n) => format!("N{}p{}", n.nt, n.prod),
    }
}

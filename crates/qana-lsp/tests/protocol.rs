...
fn hot_reload_accepts_good_configs_and_refuses_bad_ones_with_counterexamples() {
    let dir = std::env::temp_dir().join(format!("qana-lsp-test-{}", std::process::id()));
...
fn qana_documents_get_grammar_services() {
    ...
    s.handle(&json!({
        "jsonrpc": "2.0", "method": "textDocument/didOpen",
        "params": {"textDocument": {"uri": g_uri, "languageId": "qana-grammar", "text": text, "version": 1}}
    }));

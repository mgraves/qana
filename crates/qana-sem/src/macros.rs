...
use qana_grammar::green::{GreenChild, GreenNode, LIST_PROD, RUN_PROD};
use qana_grammar::model::LexGrammar;
...
fn symbol_child<'a>(
    ...
    for c in &n.children {
        ...
        let is_symbol = match c {
            ...
            GreenChild::Node(m) => m.nt != qana_grammar::green::ERROR_NT,
        };
        if is_symbol {
            if idx == k {
                let node = match c {
...
fn list_elements(n: &GreenNode, base: u32, syn: &SyntaxInfo) -> Vec<Arg> {
    if n.prod == LIST_PROD {
        ...
        for c in &n.children {
            if let GreenChild::Node(m) = c {
                if m.nt != qana_grammar::green::ERROR_NT {

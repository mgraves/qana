...
use rantlr_engine::{IncSession, Line, LineTerm};
use rantlr_sem::SemDb;
use rantlr_services::paint::{facts_at, type_hints, Painter};
use std::sync::Arc;
...
impl Limner for LiveDoc {
    ...
    fn edit(&mut self, edit: &LineEdit) -> PaintDelta {
        ...
        let outcome = self
            .session
            .edit(self.lang.sg, self.lang.tables, &[rantlr_engine::LineEdit {
                start,
                end,
                replacement,
            }])

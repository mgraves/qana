...
fn c_world() -> (rantlr_grammar::CompiledLexer, rantlr_rg::compile::LangDef, rantlr_grammar::LrTables)
{
...
fn semantic_refinement_never_changes_a_style() {
    ...
    for (l0, l1) in wave0.lines.iter().zip(both.lines.iter()) {
        ...
        // compare byte-wise.
        let bytes = |runs: &[rantlr_services::paint::Run]| {
...
fn wire_roundtrip_is_identity() {
    ...
    let items: Vec<(u32, &[rantlr_services::paint::Run])> =

...
fn emit(src: &str) -> rantlr_rg::tsgen::TsOutput {
...
fn emitted_artifacts_are_current() {
    ...
    assert_eq!(
        ...
        ts.grammar_js,
        "regenerate: cargo run -p rantlr-rg --bin rg2ts -- crates/rantlr-rg/chartlang.rg tree-sitter/chartlang"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        ts.grammar_js,
        "regenerate: cargo run -p rantlr-rg --bin rg2ts -- crates/rantlr-rg/rg.rg tree-sitter/rg"
    );
...
fn tree_sitter_generate_accepts_the_emission() {
    ...
    for (name, src) in [("chartlang", CHARTLANG_RG), ("rg", RG_RG)] {
        let dir = std::env::temp_dir().join(format!("rantlr-tsgen-{}-{name}", std::process::id()));
